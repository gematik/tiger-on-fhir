...
public class StaticFhirValidationGlue {
  ...
  private static final Map<String, ValidationModule> validationModuleCache =
  ...
  public ValidationModule supportedValidationModule(final String validationModuleId) {
    ...
    Optional<SupportedValidationModule> supportedValidationModule =
        SupportedValidationModule.fromString(normalizedId);
    if (supportedValidationModule.isPresent()) {
      validationModule =
          new ValidationModuleFactory().createValidationModule(supportedValidationModule.get());
    ...
    try {
      ...
      if (plugin == null) {
        throw new IllegalArgumentException(
            "Invalid validation module id for both SupportedValidationModule and Plugin: "
                + validationModuleId);
      }
      ...
      throw new IllegalArgumentException(
          "Something went wrong while trying to get the plugin with id: " + validationModuleId, e);
    }
  }
  ...
  @Then(
      "FHIR current request at {tigerResolvedString} is a valid {supportedValidationModule}"
          + " resource")
  @Wenn(
      "FHIR die aktuelle Anfrage im Knoten {tigerResolvedString} eine gültige"
          + " {supportedValidationModule} Ressource enthält")
  @Dann(
  ...
  @Then(
      "FHIR current request at {tigerResolvedString} is a valid {supportedValidationModule}"
          + " resource and conforms to profile {tigerResolvedString}")
  @Wenn(
      "FHIR die aktuelle Anfrage im Knoten {tigerResolvedString} eine gültige"
          + " {supportedValidationModule} Ressource, die zum Profil {tigerResolvedString} konform"
          + " ist enthält")
  @Dann(
      "FHIR prüfe die aktuelle Anfrage enthält im Knoten {tigerResolvedString} eine gültige"
          + " {supportedValidationModule} Ressource, die zum Profil {tigerResolvedString} konform"
  ...
  @Then(
      "FHIR current response at {tigerResolvedString} is a valid FHIR {supportedValidationModule}"
          + " resource")
  @Wenn(
      "FHIR die aktuelle Antwort im Knoten {tigerResolvedString} eine gültige"
          + " {supportedValidationModule} Ressource enthält")
  @Dann(
